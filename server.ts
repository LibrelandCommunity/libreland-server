/**
 * Main server implementation for the Anyland-like application.
 * This file contains multiple server instances:
 * - Main API server (authentication, area management, etc.)
 * - AreaBundles server (serving area data)
 * - ThingDefs server (serving object definitions)
 * - UGC Images server (serving user-generated content)
 */

import * as path from "node:path"
import * as fs from "node:fs/promises"
import { Elysia, t } from 'elysia'
import { AreaInfoSchema } from "./src/types/area"
...
interface HoldGeometry {
    thingId: string
    geometry: any // TODO: Define proper geometry type
}

// Types for hold geometry
interface HoldGeometryRequest {
    thingId: string
    geometry?: any // TODO: Define proper geometry type
}
...
const app = new Elysia()
    ...
    .post(
        "/area/info",
        ({ body: { areaId } }: { body: { areaId: string } }) => Bun.file(path.resolve("./data/area/info/", areaId + ".json")).json(),
    ...
    .post(
        "/placement/info",
        ({ body: { areaId, placementId } }: { body: { areaId: string, placementId: string } }) => Bun.file(path.resolve("./data/placement/info/", areaId, placementId + ".json")).json(),
    ...
    .get("/thing/info/:thingId",
        ({ params: { thingId } }: { params: { thingId: string } }) => Bun.file(path.resolve("./data/thing/info/", thingId + ".json")).json(),
    )
    .get("/thing/sl/tdef/:thingId",
        ({ params: { thingId } }: { params: { thingId: string } }) => Bun.file(path.resolve("./data/thing/def/", thingId + ".json")).json(),
    )
    .post(
        "/thing/gettags",
        ({ body: { thingId } }: { body: { thingId: string } }) => Bun.file(path.resolve("./data/thing/tags/", thingId + ".json")).json(),
    ...
    .post(
        "/gift/getreceived",
        ({ body: { userId } }: { body: { userId: string } }) => Bun.file(path.resolve("./data/person/gift/", userId + ".json")),
    ...
    .get("/forum/forum/:id", ({ params: { id } }: { params: { id: string } }) => Bun.file(path.resolve("./data/forum/forum/", id + ".json")).json())
    .get("/forum/thread/:id", ({ params: { id } }: { params: { id: string } }) => Bun.file(path.resolve("./data/forum/thread/", id + ".json")).json())
    .post(
        "/forum/forumid",
...
console.log(`🦊 UGC Images server is running at on port ${app_ugcImages.server?.port}...`)
